#!/usr/bin/env python3
"""Serve a blinded lab annotation batch and keep results separate by annotator."""
...
class Store:
    ...
    def save(self, token: str, key: str, payload: dict) -> Path:
        ...
        if payload.get("turn_completion") not in ("yes", "no", "uncertain"):
            raise ValueError("invalid_turn_completion")
        if payload.get("natural_completion") not in (None, "yes", "no", "uncertain"):
            raise ValueError("invalid_natural_completion")
...
def make_handler(store: Store):
    class Handler(BaseHTTPRequestHandler):
        server_version = "LabCtcAgreement/1"
