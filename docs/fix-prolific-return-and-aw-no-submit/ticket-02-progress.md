...
- 未使用真实凭据或生产 API；当前环境未配置可用的只读平台客户端，因此没有声称 live validation。调用方需提供实现 `list_submissions(study, page, page_size)` plus `get_submission(submission_id)` 的客户端，并将真实分页字段适配到该接口。
