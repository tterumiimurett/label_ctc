...
states['S3']['status']='ACTIVE';posts=[];entered=threading.Event();release=threading.Event();gets=[]
class API(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if self.path.startswith('/api/v1/messages/'):self.output({'results':[],'_links':{'self':{'href':self.path}}})
  elif self.path.startswith('/api/v1/submissions/?'):self.output({'results':list(states.values()),'meta':{'count':3},'_links':{'self':{'href':self.path}}})
  else:self.output(states[self.path.rstrip('/').split('/')[-1]])
 def do_POST(self):
  body=json.loads(self.rfile.read(int(self.headers['Content-Length'])));posts.append(body)
  if len(posts)==1:
   entered.set()
   if not release.wait(15):self.send_error(500);return
  self.output({'id':'M'+str(len(posts))})
 def log_message(self,*args):pass
api=ThreadingHTTPServer(('127.0.0.1',0),API);ta=threading.Thread(target=api.serve_forever);ta.start();receiver=None;tr=None
try:
 with tempfile.TemporaryDirectory() as d:
  root=Path(d);candidate=root/'candidates.jsonl';candidate.write_text(json.dumps({'candidate_key':'K','pred_is_ctc':True,'audio_verify':{'verify_is_ctc':True},'tos_audio':{'outer_url':'http://localhost/a.wav'}})+'\n')
  store=VerificationStore([],[str(candidate)],root/'data',1,3,'https://example.test/complete',False)
  for sid in states:assert store.assign({'prolific_pid':'P'+sid,'study_id':'STUDY','session_id':sid})['status']=='ok'
  ...
  approvals=ApprovalStore(root/'approval.json');approvals.save(Approval(study_id='STUDY',preview_sha256='',sessions=('S3',),historical_sessions=(),routine_enabled=True,approved_at=now[0].isoformat(),approved_by='synthetic-reviewer'))
  def controller():
  ...
  c=controller();receiver=make_activation_server(c,'secret',{'run_kind':'event','activation_boundary':boundary});tr=threading.Thread(target=receiver.serve_forever);tr.start()
  def event(eid,sid):
   raw=json.dumps({'event_type':'submission.status.change','resource_id':sid}).encode();ts=str(int(now[0].timestamp()));sig=base64.b64encode(hmac.new(b'secret',ts.encode()+raw,hashlib.sha256).digest()).decode()
  ...
  event('E1','S1');event('E2','S2');assert posts==[]
  now[0]+=timedelta(minutes=10);result={}
  def run():
   try:result['scheduler']=controller().scheduled_reassessment()
   except BaseException as exc:result['error']=repr(exc)
  runner=threading.Thread(target=run);runner.start();assert entered.wait(10),'No first HTTP POST reached controlled server'
  ...
  release.set();runner.join(10);assert not runner.is_alive(),'Deadlock after disable';assert 'error' not in result,result
  states['S3']['status']='RETURNED';later=event('E3','S3');restart=controller().scheduled_reassessment()
  ledger=JsonContactLedger(root/'contacts.json').read();assignments=json.loads(store.assignments_path.read_text())
