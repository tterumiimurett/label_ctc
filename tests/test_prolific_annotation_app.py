...
class ConversationAnnotationAppTest(unittest.TestCase):
    ...
    def test_frontend_supports_multiple_phenomena_without_removed_ctc_fields(self) -> None:
        ...
        self.assertIn("context.createChannelSplitter(2)", javascript)
        self.assertIn("await setPlaybackChannel(Number(segment.channel))", javascript)
        self.assertIn("await setPlaybackChannel()", javascript)
        ...
        self.assertIn('id=\"audio-status\"', html)
        self.assertIn("Audio failed to load", javascript)
