...
class Ticket08Test(unittest.TestCase):
    ...
    def setup(self, production=False):
        ...
        trigger = Mock()
        trigger.periodic.return_value = {'report': {'status': 'ok', 'writes_performed': False, 'submissions': []}}
        adapter = Mock()
    ...
    def test_provenance_is_durable_and_unknown_is_manual(self):
        d, r, c = self.setup()
        self.assertEqual(c.derive_candidate_origin({'run_kind': 'event', 'event_id': 'E', 'activation_boundary': '1970-01-01', 'study_id': 'STUDY'}, accepted_event={'event_id': 'E', 'study_id': 'STUDY', 'event_timestamp': 100, 'activation_timestamp': 0}), 'new')
        self.assertEqual(c.derive_candidate_origin({'run_kind': 'backfill', 'historical_snapshot': True}), 'historical')
        self.assertEqual(c.derive_candidate_origin({'run_kind': 'manual'}), 'unknown')
        d.cleanup()
    ...
    def test_production_requires_explicit_approval(self):
        ...
        with self.assertRaises(PermissionError):
            c.execute(provenance_context={'run_kind': 'event', 'event_id': 'E'})
        d.cleanup()

    def test_preview_uses_actual_action_name(self):
        d, r, c = self.setup()
        out = c.preview({'submissions': [{'session_id': 'S', 'study_id': 'STUDY', 'participant_id': 'P', 'proposed_action': 'release_claim_proposal'}]})
        self.assertEqual(out['actions'][0]['action'], 'release_claim')
        d.cleanup()
    ...
    def test_signed_http_event_real_client_and_verification_store_release_claim(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                client = ProlificSubmissionClient('isolated', 'http://127.0.0.1:' + str(port) + '/api/v1', retries=0)
                ...
                self.assertEqual(result['origin'], 'new')
                self.assertEqual(result['results'][0]['outcome']['action'], 'released_claim')
            finally:
                server.shutdown()
                server.server_close()
                thread.join()
...
class FullHttpMatrixTest(unittest.TestCase):
    ...
    def test_receiver_http_signed_returned_final_archives_real_store(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            at = threading.Thread(target=api.serve_forever)
            at.start()
            receiver = None
            rt = None
            try:
                base = 'http://127.0.0.1:%d/api/v1' % api.server_address[1]
                client = ProlificSubmissionClient('isolated', base, retries=0)
                trigger = ReconciliationTrigger(client, root / 'data', 'STUDY', JsonTriggerStore(root / 'events.json'))
                controller = ActivationController(trigger=trigger, store=store, ledger=JsonContactLedger(root / 'contacts.json'), adapter=RealApiAdapter(client, root / 'data', 'STUDY'), journal=ActionJournal(root / 'actions.jsonl'), study_id='STUDY', approvals=ApprovalStore(root / 'approval.json'), activation_boundary='1970-01-01T00:00:00Z', production_enabled=True)
                controller.approvals.save(Approval('STUDY', '', (), (), (), True, 'fixture', 'test'))
                receiver = make_activation_server(controller, 'secret', {'run_kind': 'event', 'activation_boundary': '1970-01-01', 'study_id': 'STUDY'})
                rt = threading.Thread(target=receiver.serve_forever)
                rt.start()
                body = json.dumps({'event_type': 'submission.status.change', 'resource_id': 'S1'}).encode()
                ts = '100'
                ...
                self.assertFalse((root / 'data/submissions/S1.json').exists())
                self.assertTrue(list((root / 'data/excluded_submissions').glob('*/prolific_returned/S1.json')))
            finally:
                if receiver:
                    receiver.shutdown()
    ...
    def test_duplicate_signed_event_and_kill_switch_do_not_repeat_effect(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            first = journal.begin('recipient_message', {'session_id': 'S1'})
            self.assertIsNotNone(first)
            journal.finish(first, 'unknown_delivery')
            journal.disable('operator stop')
            self.assertIsNone(journal.begin('recipient_message', {'session_id': 'S2'}))
    ...
    def _run_real_timeout(self, with_result: bool):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            candidate_path.write_text(json.dumps({'candidate_key': 'timeout-k', 'pred_is_ctc': True, 'audio_verify': {'verify_is_ctc': True}, 'tos_audio': {'outer_url': 'https://controlled.test/a.wav'}}) + '\n', encoding='utf-8')
            store = VerificationStore([], [str(candidate_path)], root / 'data', 1, 1, 'https://controlled.test/complete', False)
            assignment = store.assign({'prolific_pid': 'P-TIMEOUT', 'study_id': 'STUDY', 'session_id': 'TIMEOUT-1'})
            self.assertEqual(assignment['status'], 'ok')
            raw_result = None
            if with_result:
                task = assignment['tasks'][0]
                payload = {'schema_version': 'ctc-verification-v1', 'worker': {'prolific_pid': 'P-TIMEOUT', 'study_id': 'STUDY', 'session_id': 'TIMEOUT-1'}, 'assignment': assignment['assignment'], 'tasks': [{'candidate_id': task['candidate_id'], 'task_id': task.get('task_id') or task.get('id') or task['candidate_id'], 'relevant_interruption': False}]}
                self.assertEqual(store.submit(payload)['status'], 'ok')
                raw_result = (root / 'data' / 'submissions' / 'TIMEOUT-1.json').read_bytes()
            platform_state = {'id': 'TIMEOUT-1', 'study_id': 'STUDY', 'participant': 'P-TIMEOUT', 'status': 'TIMED OUT'}

            class ControlledApi(BaseHTTPRequestHandler):

                def do_GET(self):
                    if '?' in self.path:
                        value = {'results': [platform_state], 'meta': {'count': 1}, '_links': {'self': {'href': self.path}}}
                    else:
            ...
            try:
                ...
                trigger = ReconciliationTrigger(client, root / 'data', 'STUDY', JsonTriggerStore(root / 'events.json'))
                controller = ActivationController(trigger=trigger, store=store, ledger=JsonContactLedger(root / 'contacts.json'), adapter=RealApiAdapter(client, root / 'data', 'STUDY'), journal=ActionJournal(root / 'actions.jsonl'), study_id='STUDY', approvals=ApprovalStore(root / 'approval.json'), production_enabled=True)
                controller.approvals.save(Approval('STUDY', '', (), (), (), True, 'fixture', 'test'))
                receiver = make_activation_server(controller, 'controlled-secret', {'run_kind': 'event', 'activation_boundary': '1970-01-01T00:00:00Z', 'study_id': 'STUDY'})
                receiver_thread = threading.Thread(target=receiver.serve_forever)
                receiver_thread.start()
                body = json.dumps({'event_type': 'submission.status.change', 'resource_id': 'TIMEOUT-1'}).encode('utf-8')
                timestamp = '200'
                signature = base64.b64encode(hmac.new(b'controlled-secret', timestamp.encode() + body, hashlib.sha256).digest()).decode()
                request = urllib.request.Request('http://127.0.0.1:%d/' % receiver.server_address[1], data=body, method='POST', headers={'X-Prolific-Request-Signature': signature, 'X-Prolific-Request-Timestamp': timestamp, 'X-Event-ID': 'timeout-event-1', 'X-Timestamp': timestamp, 'Content-Type': 'application/json'})
                response = json.loads(urllib.request.urlopen(request).read())
...
class RoutinePositiveHttpTest(unittest.TestCase):
    ...
    def test_approved_routine_new_missing_result_waits_then_posts_once(self):
        ...
        from prolific.ctc_verification_app.activation import ActionJournal, ActivationController, Approval, ApprovalStore, RealApiAdapter, make_activation_server
        from prolific.ctc_verification_app.app import VerificationStore
        from prolific.ctc_verification_app.contact_candidates import JsonContactLedger, VerifiedMessageScope, APPROVED_MESSAGE
        from prolific.ctc_verification_app.reconciliation import ProlificSubmissionClient
        from prolific.ctc_verification_app.triggers import JsonTriggerStore, ReconciliationTrigger
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            candidate_path = root / 'candidates.jsonl'
            candidate_path.write_text(json.dumps({'candidate_key': 'routine-k', 'pred_is_ctc': True, 'audio_verify': {'verify_is_ctc': True}, 'tos_audio': {'outer_url': 'https://controlled.test/a.wav'}}) + '\n', encoding='utf-8')
            store = VerificationStore([], [str(candidate_path)], root / 'data', 1, 1, 'https://controlled.test/complete', False)
            assignment = store.assign({'prolific_pid': 'P-NEW', 'study_id': 'STUDY', 'session_id': 'NEW-1'})
            self.assertEqual(assignment['status'], 'ok')
            now = [datetime(2026, 1, 1, tzinfo=timezone.utc)]
            message_posts = []
            platform_state = {'id': 'NEW-1', 'study_id': 'STUDY', 'participant': 'P-NEW', 'status': 'AWAITING REVIEW', 'started_at': '2026-01-01T00:00:00Z'}
            ...
            receiver_thread = None
            try:
                client = ProlificSubmissionClient('controlled-token', 'http://127.0.0.1:%d/api/v1' % api.server_address[1], retries=0)
                scope = VerifiedMessageScope('RESEARCHER', 'WORKSPACE', datetime(2025, 12, 3, tzinfo=timezone.utc), datetime(2026, 1, 2, tzinfo=timezone.utc), True, 'controlled full message visibility', now[0], now[0] + timedelta(hours=1))
                ...
                controller = ActivationController(trigger=trigger, store=store, ledger=JsonContactLedger(root / 'contacts.json'), adapter=adapter, journal=ActionJournal(root / 'actions.jsonl'), study_id='STUDY', approvals=approvals, production_enabled=True, clock=lambda: now[0], activation_boundary='1970-01-01T00:00:00Z')
                initial_report = trigger.periodic()['report']
                preview = controller.preview(initial_report)
                approvals.save(Approval('STUDY', '', (), (), (), True, now[0].isoformat(), 'routine-policy-reviewer'))
                receiver = make_activation_server(controller, 'routine-secret', {'run_kind': 'event', 'activation_boundary': '1970-01-01T00:00:00Z', 'study_id': 'STUDY'})
                receiver_thread = threading.Thread(target=receiver.serve_forever)
                receiver_thread.start()
                body = json.dumps({'event_type': 'submission.status.change', 'resource_id': 'NEW-1'}).encode('utf-8')
                timestamp = '100'
                signature = base64.b64encode(hmac.new(b'routine-secret', timestamp.encode() + body, hashlib.sha256).digest()).decode()
                request_headers = {'X-Prolific-Request-Signature': signature, 'X-Prolific-Request-Timestamp': timestamp, 'X-Event-ID': 'routine-event-1', 'X-Timestamp': timestamp, 'Content-Type': 'application/json'}
                request = urllib.request.Request('http://127.0.0.1:%d/' % receiver.server_address[1], data=body, method='POST', headers=request_headers)
