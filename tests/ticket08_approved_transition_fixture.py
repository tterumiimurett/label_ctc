...
try:
 with tempfile.TemporaryDirectory() as d:
  root=Path(d);candidate=root/'candidates.jsonl';candidate.write_text(json.dumps({'candidate_key':'V00_S0001_I00000001|P1|P2|11.000|21.000|target','pred_is_ctc':True,'audio_verify':{'verify_is_ctc':True},'tos_audio':{'outer_url':'http://127.0.0.1/seamless_ctc_V00_S0001_I00000001_001100_002100.wav'}})+'\n')
  ...
  if mode=='answer':
   ...
   submit_result=store.submit(payload); assert submit_result['status']=='ok',submit_result
  if mode=='approved': state['status']='APPROVED'
  ...
  if mode!='positive': assert ledger.read()['sessions']['S1']['state']=='manual_review','Missing durable manual disposition'
finally:
